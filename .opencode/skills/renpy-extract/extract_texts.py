...
class RenPyExtractor:
    ...
    def scan_engine_builtins(self, renpy_common_dir: Path) -> list[dict]:
        """Scan Ren'Py common/ directory for _(\"text\") built-in strings."""
        engine_blocks: Dict[str, dict] = {}
        common_files = sorted(
            p for p in renpy_common_dir.rglob("*.rpy")
            if p.name.startswith('00')
        )

        print(f"Scanning engine built-ins in {renpy_common_dir}...")
        ...
        for f in common_files:
            ...
            for i, line in enumerate(content.split('\n')):
                ...
                for m in re.finditer(r'_\("([^"]+)"\)', stripped):
                    ...
                    if text:
                        dk = self._dedup_key(text)
                        if dk not in self._seen_originals and dk not in engine_blocks:
                            engine_blocks[dk] = {
                                'id': f"engine_{_hash(f'{f.name}:{i+1}:{text}')}",
                                'original': text,
                                'translated': self._existing(text),
                                'type': 'engine_string',
                                'source_file': str(f.relative_to(renpy_common_dir.parent.parent)),
                                'source_line': i + 1,
                            }

        print(f"  Engine built-in strings: {len(engine_blocks)}")
        return list(engine_blocks.values())
...
def _collect_old_strings_in_tl(tl_dir: Path, exclude: str = None) -> set[str]:
    """Scan all .rpy files in tl_dir (except exclude) and return set of old strings."""
    olds: set[str] = set()
    for rpy_file in sorted(tl_dir.rglob("*.rpy")):
        if exclude and rpy_file.name == exclude:
            continue
        try:
            content = rpy_file.read_text(encoding='utf-8')
            for line in content.split('\n'):
                ...
                if s.startswith('old '):
                    m = re.match(r'old "(.*)"\s*$', s)
                    if m:
                        olds.add(_unescape(m.group(1)).strip())
        except Exception:
            pass
    return olds


def generate_engine_rpy(engine_blocks: list[dict], tl_dir: Path, existing_translations: dict[str, str] = None) -> list[str]:
    """Generate renpy_common.rpy with built-in strings.

    Translations are sourced (in priority order):
    1. existing_translations dict (from all tl/ru/ files)
    2. existing renpy_common.rpy file on disk
    3. original text (fallback)

    Skips any old string that already exists in other tl/ru/ .rpy files
    to avoid Ren'Py duplicate translation exceptions.
    """
    if not engine_blocks:
        return []

    # Collect old strings already present in other tl/ru/ files
    existing_olds = _collect_old_strings_in_tl(tl_dir, exclude='renpy_common.rpy')

    # Filter out blocks whose old string already exists elsewhere
    skipped = 0
    filtered = []
    for bd in engine_blocks:
        orig_text = bd['original'].strip()
        if orig_text in existing_olds:
            skipped += 1
            continue
        filtered.append(bd)

    if not filtered:
        print("  All engine built-in strings already exist in other tl/ru/ files, nothing to generate.")
        return []

    if skipped:
        print(f"  Skipped {skipped} string(s) already present in other tl/ru/ files")

    engine_blocks = filtered
    engine_blocks.sort(key=lambda b: b['original'].lower())
    out = tl_dir / 'renpy_common.rpy'
    generated = ['renpy_common.rpy']
            ...
            for line in content.split('\n'):
                ...
                if in_translate and s.startswith('old '):
                    ...
                    if m:
                        current_orig = _unescape(m.group(1))
                    continue
                if in_translate and current_orig and s.startswith('new '):
                    m = re.match(r'new "(.*)"\s*$', s)
                    if m:
                        t = _unescape(m.group(1))
                        if t and t != current_orig:
                            existing_from_file[current_orig.strip()] = t
                    current_orig = None
                    continue
                if in_translate and s and not s.startswith(('old ', 'new ', '#')):
                    in_translate = False
    ...
    # Merge: existing_translations (higher priority) > existing_from_file > original
    with open(out, 'w', encoding='utf-8') as f:
        f.write("# -*- encoding: utf-8 -*-\n")
        f.write("# Ren'Py Engine Built-in Strings\n")
        f.write("# Extracted from renpy/common/00*.rpy\n\n")
        f.write("translate ru strings:\n\n")
        for bd in engine_blocks:
            o = _esc(bd.get('original', ''))
            orig_text = bd['original'].strip()
            t_raw = ''
            if existing_translations and orig_text in existing_translations:
                t_raw = existing_translations[orig_text]
            elif orig_text in existing_from_file:
                t_raw = existing_from_file[orig_text]
            t = _esc(t_raw if t_raw else bd.get('original', ''))
            f.write(f"    # Source: {bd.get('source_file', '')}:{bd.get('source_line', '')}\n")
            f.write(f'    old "{o}"\n')
            f.write(f'    new "{t}"\n\n')
...
def _clean_engine_duplicates_from_screens(tl_dir: Path):
    """Remove built-in confirm strings from screens.rpy if they exist in renpy_common.rpy."""
    screens = tl_dir / 'screens.rpy'
    common = tl_dir / 'renpy_common.rpy'
    if not screens.exists() or not common.exists():
        return

    # Collect old strings from renpy_common.rpy
    common_olds = set()
    try:
    ...
    # Filter out duplicate old/new pairs from screens.rpy
    try:
    ...
    for i, line in enumerate(lines):
        ...
        if s.startswith('old '):
            m = re.match(r'old "(.*)"\s*$', s)
            if m and m.group(1) in common_olds:
                # Skip this old + next new line
                skip_next = True
                removed += 1
                continue
    ...
    if removed:
        ...
        print(f"  Removed {removed} duplicate built-in string(s) from screens.rpy")


def _find_renpy_common_dir(project_root: Path) -> Path | None:
    """Find the Ren'Py SDK common/ directory."""
    candidates = [
        project_root / 'renpy' / 'common',
        project_root / 'renpy-8.5.2-sdk' / 'renpy' / 'common',
    ]
    for d in sorted(project_root.parent.rglob('renpy*/renpy/common')):
        candidates.append(d)
    for d in sorted(project_root.parent.rglob('renpy/common')):
        candidates.append(d)
    for c in candidates:
        if c.exists() and any(c.glob('00*.rpy')):
            return c
    return None


def count_engine_translations(tl_dir: Path) -> tuple[int, int]:
    """Count translated vs total in renpy_common.rpy."""
...
def verify_integrity(original_dir: Path, tl_dir: Path, renpy_common_dir: Path = None):
    ...
    # Engine built-ins
    engine_total, engine_done = count_engine_translations(tl_dir)
    if engine_total:
        total += engine_total
        done += engine_done
    ...
    if engine_total:
        print(f"  Engine builtins: {engine_total} ({100*engine_done//engine_total if engine_total else 0}%)")
    print("=" * 40)
    return total, done
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("  extract-builtins  Scan Ren'Py engine built-in strings")
    ...
    if cmd == 'extract':
    ...
    elif cmd == 'extract-builtins':
        renpy_common = _find_renpy_common_dir(project_root)
        if not renpy_common:
            print("ERROR: Ren'Py common/ directory not found!")
            print("Looked in: renpy/common, renpy-8.5.2-sdk/renpy/common, etc.")
            return
        extractor = RenPyExtractor(str(game_dir))
        extractor.set_existing_translations(tl_dir)
        blocks = extractor.scan_engine_builtins(renpy_common)
        generated = generate_engine_rpy(blocks, tl_dir, extractor._existing_translations)
        if generated:
            print(f"Generated {generated[0]} ({len(blocks)} strings)")
        else:
            print("No engine built-in strings found.")
        # Remove duplicate built-in entries from screens.rpy if present
        _clean_engine_duplicates_from_screens(tl_dir)
        print("Done!")
        ...
        if not tl_dir.exists():
            ...
            return
        renpy_common = _find_renpy_common_dir(project_root)
        verify_integrity(game_dir, tl_dir, renpy_common)
