...
tools/
  ...
  ref_prepare.py               # кандидат -> реф: сжатие пауз (>0.7с -> 0.35с), окно 10с
                               #   от начала звука, loudnorm -16 LUFS/TP -1.5 (стандарт)
...
(одним прогоном: mp3 -> нарезка 10с -> чистка -> in_progress/{Имя}.wav).

ПОДГОТОВКА РЕФА из отобранного кандидата: tools/ref_prepare.py
(сжатие пауз > 0.7с до 0.35с -> окно 10с от начала звука -> loudnorm
-16 LUFS / TP -1.5). Единый уровень громкости для ВСЕХ рефов.
...
Правила: каст в `voice_candidates/{Имя}/{Имя}.yaml` (instruct_en — ФИЗИОГНОМИКА
тела для генерации: пол, возраст с «лет», телосложение/лицо/осанка/поступь —
БЕЗ описания голоса (типаж вырисовывается из тела), instruct_ru — русский
перевод для ревью, texts — рус. фразы с ЯВНЫМ признаком пола; файл = контракт);
