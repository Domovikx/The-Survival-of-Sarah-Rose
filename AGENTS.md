...
тембра для генерации, instruct_ru — русский перевод для ревью, texts — рус.
фразы с ЯВНЫМ признаком пола «я пошёл/пошла»; файл = контракт); клип должен быть ≥10с — тул сам добирает длину (slow-инструкция →
...
- Параметры вариативности влияют ТОЛЬКО МЕЖДУ кандидатами: `variations`
  (через `{style}` или «; ») и `temperature_span`/`top_p_span` выбираются
  ОДИН раз на клип и применяются ко всей фразе целиком.
- Риск дрейфа голоса ВНУТРИ клипа создают:
  - длина текста > 20-30с (архитектурный дрейф CV3) — реплики игры 5-15с
    безопасны;
  - слишком высокие температура (>1.1) и top_p (1.0) — ограничены
    дефолтами temperature_span [0.75, 1.05] и top_p_span [0.9, 1.0];
  - flow-temp (voice_batch) — выше 1.0 тембр «гуляет» на длинных фразах.
- RAS (штраф повторов) держим — он создан для стабильности LLM.
- В yaml можно переопределить: `temperature_span: [a, b]`,
  `top_p_span: [a, b]`, `variations: [...]`.
